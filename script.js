...
// ── 6. CONTACT FORM — opens default email client with pre-filled message
...
contactForm.addEventListener('submit', (e) => {
  ...
  const subject  = encodeURIComponent(`Message from ${nameVal} via Portfolio`);
  const body     = encodeURIComponent(
    `Name: ${nameVal}\nEmail: ${emailVal}\n\nMessage:\n${messageVal}`
  );
  const mailto   = `mailto:dola.das@cse.kuet.ac.bd?subject=${subject}&body=${body}`;
